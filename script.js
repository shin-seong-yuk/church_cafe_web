...
(function init() {
  ...
  const sundays = enumerateSundays(today, target);
  if (!sundays.length) {
    wrap.innerHTML = '<div class="small" style="color:#64748b">표시할 일요일이 없습니다.</div>';
    return;
  }
  ...
  sortedKeys.forEach((key, groupIdx) => {
    ...
    dates.forEach((d, idx) => {
      ...
      btn.className =
        'btn-date' +
        (startOfDay(d).getTime() === startOfDay(today).getTime() ? ' is-today' : '') +
        (groupIdx === sortedKeys.length - 1 && idx === dates.length - 1 ? ' is-last' : '');
      btn.type = 'button';
      btn.textContent = labelKoreanSunday(d);
      btn.setAttribute('aria-label', `${d.getFullYear()}년 ${d.getMonth()+1}월 ${d.getDate()}일(일) 선택`);
